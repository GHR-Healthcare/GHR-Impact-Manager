...
def ensure_schema(cursor):
    ...
    cursor.execute("""
        IF NOT EXISTS (
            ...
            WHERE name = 'pm_mappings' AND schema_id = SCHEMA_ID('impactmgr')
        )
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if req.method == 'GET':
            cursor.execute('SELECT facility, pm_name FROM impactmgr.pm_mappings ORDER BY facility')
            mappings = [
            ...
            for m in mappings:
                ...
                if facility and pm_name:
                    cursor.execute(
                        'INSERT INTO impactmgr.pm_mappings (facility, pm_name) VALUES (?, ?)',
                        facility, pm_name
                    )
                    saved += 1
