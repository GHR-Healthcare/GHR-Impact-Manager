...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        #
        # Facility resolution: workorders only carry a coarse [Default Work
        # Site Name] (Cooper Main GL codes like "Nursing Administration-
        # 01-6601-5400") that doesn't distinguish sub-facilities. The JOBS
        # table has the proper [Facility] ("Cape Regional Medical Center",
        # "Cooper University Healthcare") plus [Facility Code]. Join via
        # Work Order Id + matching cost-center name to pick the right job
        # row (some workorders have stale duplicate job records pointing
        # at the wrong facility — the cost-center match disambiguates).
        # Falls back to Default Work Site Name when no job match exists.
        # ============================================================
        try:
            cursor.execute('''
                SELECT DISTINCT
                    ...
                    COALESCE(j.[Facility], wo.[Default Work Site Name]) AS facility,
                ...
                LEFT JOIN (
                    SELECT DISTINCT
                        [Work Order Id],
                        [Charge Code - Cost Center Name],
                        [Facility]
                    FROM dbo.STAGING_VNDLY_JOBS
                    WHERE [Facility] IS NOT NULL
                ) j ON j.[Work Order Id] = wo.[Work Order Id]
                    AND j.[Charge Code - Cost Center Name] = wo.[Default Work Site Name]
        ...
        # Same JOBS-table lookup as the assignment query above so shifts
        # roll up to the right sub-facility (Cape Regional vs Cooper
        # University Healthcare) instead of a Cooper Main GL code.
        # ============================================================
        try:
            cursor.execute(f'''
                SELECT
